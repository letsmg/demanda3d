...
function fillTestCredentials(acc: TestCarrierAccount) {
    form.email = acc.email;
    form.password = 'Mudar@123';
}

function submit() {
    form.post('/login_carrier', { preserveState: true });
}
</script>
        ...
        <Card class="border-amber-500 bg-amber-50/80 shadow-sm">
            <CardHeader class="pb-2 text-center">
                <CardTitle class="text-xl text-amber-900"
                    >Entrar como Transportadora</CardTitle
                >
                <CardDescription class="text-amber-600"
                    >Acesse sua conta de transportadora para gerenciar
                    fretes</CardDescription
                >
            </CardHeader>
            <CardContent class="space-y-6">
                <div
                    class="rounded-lg border border-dashed border-amber-500/50 bg-amber-100/50 p-3"
                >
                    <p
                        class="mb-2 text-center text-xs font-medium text-amber-700"
                    >
                ...
                <form @submit.prevent="submit" class="flex flex-col gap-6">
                    <div class="grid gap-4">
                        ...
                        <div class="flex items-center justify-between">
                            <Label
                                for="remember"
                                class="flex items-center space-x-3 text-amber-700"
                            >
                                <Checkbox
                                    id="remember"
                        ...
                        <Button
                            type="submit"
                            class="hover:bg-amber-910 mt-2 w-full bg-amber-500 font-semibold text-amber-950"
                            :tabindex="4"
                            :disabled="form.processing"
                        >
                            <Spinner v-if="form.processing" /> Entrar como
                            Transportadora
                        </Button>
                    </div>

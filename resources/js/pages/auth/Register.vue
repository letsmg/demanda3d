...
function handleFill() {
    ...
    const passwordConfInput = document.querySelector<HTMLInputElement>('input[name="password_confirmation"]');
    ...
    <div class="mx-auto flex w-full flex-col justify-center py-12 sm:w-[460px]">
        <Card class="border-amber-500 bg-amber-50/80 shadow-sm">
            <CardHeader class="text-center pb-2">
                ...
                <CardDescription class="text-amber-600">Cadastre-se para começar a vender na plataforma</CardDescription>
            </CardHeader>
            <CardContent>
                <FormTestHelper
                    ...
                    @clear="handleClear"
                />
                <Form v-bind="store.form()" :reset-on-success="['password', 'password_confirmation']" v-slot="{ errors, processing }" class="mt-3 flex flex-col gap-6">
                    ...
                    <div class="grid gap-4">
                        <div class="grid gap-2">
                            ...
                            <Input id="email" type="email" name="email" required :tabindex="1"
                        ...
                        <div class="grid gap-2">
                            ...
                            <PasswordInput id="password" name="password" v-model:show="showPassword" required :tabindex="2" placeholder="Mínimo 8 caracteres" />
                            <InputError :message="errors.password" />
                        </div>
                        <div class="grid gap-2">
                            <Label for="password_confirmation" class="text-amber-800">Confirmar Senha *</Label>
                            <PasswordInput id="password_confirmation" name="password_confirmation" v-model:show="showPassword" required :tabindex="3" placeholder="Repita a senha" />
                        </div>
                        <p class="text-xs text-amber-600">Após o cadastro, você será direcionado para completar os dados da sua loja e perfil.</p>
                        <div class="rounded-md bg-amber-100 p-3 space-y-2">
                            <div class="flex items-start gap-2">
                                <Checkbox id="accept_terms_v" v-model:checked="acceptTerms" :tabindex="4" class="border-amber-900" />
                            ...
                            <div class="flex items-start gap-2">
                                <Checkbox id="accept_privacy_v" v-model:checked="acceptPrivacy" :tabindex="5" class="border-amber-900" />
                        ...
                        <div class="flex gap-2">
                            <Button type="submit" class="flex-1 bg-amber-500 font-semibold text-amber-950 hover:bg-amber-910" :tabindex="6" :disabled="processing">
                                <Spinner v-if="processing" /> Criar Conta de Vendedor
                            </Button>
                            <Button type="reset" variant="outline" class="border-amber-900 text-amber-700 hover:bg-amber-50" :tabindex="7">
                                Limpar
                            </Button>
                        </div>
                    </div>

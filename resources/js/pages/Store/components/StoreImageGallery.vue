    ...
    <Dialog :open="product !== null" @update:open="emit('update:open', $event)">
        <DialogContent class="max-w-3xl">
            <DialogHeader>
                ...
                <DialogDescription v-if="product?.description">{{ product.description }}</DialogDescription>
            </DialogHeader>
            <div v-if="product" class="relative">
                ...
                <button v-if="product.images && product.images.length > 1"
                    ...
                    @click="emit('prev-image')"><ChevronDown class="h-5 w-5 rotate-90 text-gray-600" /></button>
                <button v-if="product.images && product.images.length > 1"
                    type="button"
                    class="absolute top-1/2 right-2 -translate-y-1/2 rounded-full bg-white/80 p-2 shadow hover:bg-white"
                    @click="emit('next-image')"><ChevronDown class="h-5 w-5 -rotate-90 text-gray-600" /></button>
                <div v-if="product.images && product.images.length > 1" class="mt-4 flex justify-center gap-2">
                ...
                <div class="mt-4 space-y-3 rounded-lg bg-amber-50 p-4">
                    <div class="flex items-center justify-between">
                        <div>
                            <p class="text-2xl font-bold text-emerald-700">{{ formatPrice(product.sale_price) }}</p>
                            <p class="text-sm text-amber-600">{{ product.tenant?.fantasy_name || 'Vendedor' }}</p>
                            <div v-if="product.tenant?.rating_count > 0" class="mt-1 flex items-center gap-1">
                                <Star class="h-4 w-4 fill-amber-900 text-amber-900" />
                                <span class="text-sm font-medium text-amber-700">{{ product.tenant.rating_average }}</span>
                    ...
                    <a :href="`/store/${product.slug}`"
                        ...
                        @click="emit('close')"><ExternalLink class="h-4 w-4" /> Ver mais detalhes</a>
                </div>
            </div>
        </DialogContent>
    </Dialog>

...
import { Package, ChevronRight, ShoppingBag } from '@lucide/vue';
...
<template>
    ...
    <h1 class="mb-6 text-2xl font-bold tracking-tight text-amber-900">Meus Pedidos</h1>

    <Card v-if="orders.data.length === 0">
        <CardHeader class="text-center py-12">
            ...
            <CardDescription>Você ainda não fez nenhum pedido. Visite nossa loja para começar.</CardDescription>
            <Link href="/store" class="mt-4 inline-flex items-center gap-2 rounded-md bg-amber-600 px-4 py-2 text-sm font-medium text-white hover:bg-amber-700">
                ...
                <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div class="flex-1">
                        <div class="mb-2 flex items-center gap-3">
                            ...
                            <h3 class="font-semibold text-amber-900">Pedido #{{ String(order.id).padStart(6, '0') }}</h3>
                            <Badge :class="statusColor(order.status)" variant="outline">
                        ...
                        <div class="ml-8 space-y-1 text-sm">
                            ...
                            <p v-if="order.delivery_date" class="text-muted-foreground">
                                Entrega prevista: {{ new Date(order.delivery_date).toLocaleDateString('pt-BR') }}
                            </p>
                            <p v-if="order.contracted_description" class="text-muted-foreground line-clamp-2">
                    ...
                    <div class="text-right sm:min-w-[120px]">
                        <p class="text-lg font-bold text-amber-900">
                            R$ {{ Number(order.amount_total || order.price).toLocaleString('pt-BR', { minimumFractionDigits: 2 }) }}
                        </p>
        ...
        <div v-if="orders.last_page > 1" class="flex items-center justify-center gap-2 pt-4">
            <Link
                ...
                :class="{
                    ...
                    'text-amber-700 hover:bg-amber-100': !link.active && link.url,
                    'pointer-events-none text-muted-foreground opacity-50': !link.url,
                }"
            />
        </div>
    </div>

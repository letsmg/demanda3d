...
<template>
    ...
    <div class="space-y-6 p-4 md:p-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold tracking-tight md:text-3xl">Fornecedores</h1>
                <p class="text-sm text-muted-foreground">Gerencie seus fornecedores de insumos</p>
            </div>
            <Button as-child>
                <Link href="/suppliers/create"><Plus class="mr-2 h-4 w-4" />Novo Fornecedor</Link>
            </Button>
        </div>
        ...
        <Card>
            <CardHeader>
                ...
                <CardDescription>{{ suppliers.meta?.total || suppliers.data?.length || 0 }} fornecedores cadastrados</CardDescription>
            </CardHeader>
            <CardContent>
                        ...
                        <tbody>
                            <tr v-for="supplier in (Array.isArray(suppliers) ? suppliers : suppliers.data)" :key="supplier.id" class="border-b hover:bg-muted/50">
                                ...
                                <td class="py-3 px-4 text-right">
                                    ...
                                    <Button variant="ghost" size="icon" @click="deleteSupplier(supplier.id)">

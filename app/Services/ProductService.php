...
class ProductService
{
    ...
    public function create(array $data): Product
    {
        ...
        // Update canonical_url after slug is generated by model
        if (empty($product->canonical_url) && $product->slug) {
            $product->updateQuietly(['canonical_url' => route('store.detail', ['slug' => $product->slug])]);
        }
    ...
    public function update(Product $product, array $data): Product
    {
        ...
        // Regenerate canonical_url if slug changed or was empty
        if (empty($product->canonical_url) || $product->wasChanged('slug')) {
            $product->updateQuietly(['canonical_url' => route('store.detail', ['slug' => $product->slug])]);
        }
    ...
    private function autoGenerateSeoFields(array $data, ?Product $existing): array
    {
        ...
        // canonical_url: gera URL canônica baseada no slug
        if ($shouldGenerate('canonical_url')) {
            $slug = $data['slug'] ?? ($existing?->slug ?? '');
            if ($slug) {
                $data['canonical_url'] = route('store.detail', ['slug' => $slug]);
            }
        }
